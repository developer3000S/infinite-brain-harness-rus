...
Требования: git, AI-агент для программирования ([Qwen Code](https://github.com/QwenLM/qwen-code) или [Claude Code](https://claude.com/claude-code) — основная среда выполнения), аккаунт GitHub, и желательно [GitHub CLI](https://cli.github.com/) (`gh`) для аутентификации. `/start` проверяет всё это и сообщает, чего не хватает.

1. Клонируйте этот репозиторий туда, где хотите иметь корень, и откройте его в Qwen Code или Claude Code:
...
3. Перезапустите Qwen Code или Claude Code, когда `/start` скажет об этом: собственные команды мозгов загружаются только после перезапуска.
...
Собственные команды, навыки, агенты и правила каждого мозга копируются в этот корень через `/sync`, поэтому они работают как слеш-команды здесь (после перезапуска Qwen Code или Claude Code). `.claude/CAPABILITIES.md` (для Claude Code) и `.qwen/CAPABILITIES.md` (для Qwen Code), регенерируемые при синхронизации, перечисляют, что содержит каждый мозг, чтобы вы направляли работу в правильный.
...
| Путь | Что это |
|---|---|
| `QWEN.md` | основная ориентация для Qwen Code |
| `CLAUDE.md`, `AGENTS.md` | ориентация для Claude Code и Codex; зеркальные копии друг друга (устаревшее) |
| `.qwen/commands/` | команды workspace для Qwen Code (`start`, `sync`, `save`, `promote-to-department`, `workspace-help`, `register-repo`) |
| `.qwen/refresh-commands.sh` | генератор копирования команд и индекса возможностей для Qwen Code, запускаемый `/start` и `/sync` |
| `.qwen/settings.json` | защиты разрешений workspace для Qwen Code (без фиксации модели: применяется ваша модель по умолчанию) |
| `.qwen/brains.conf.example` | пример конфигурации мозгов для Qwen Code |
| `.qwen/QWEN.local.md.example` | пример локального файла заметок для Qwen Code |
| `.claude/commands/` | команды workspace для Claude Code (`start`, `sync`, `save`, `promote-to-department`, `workspace-help`, `register-repo`) (устаревшее) |
| `.claude/refresh-commands.sh` | генератор копирования команд и индекса возможностей для Claude Code, запускаемый `/start` и `/sync` (устаревшее) |
| `.claude/settings.json` | защиты разрешений workspace для Claude Code (без фиксации модели: применяется ваша модель по умолчанию) (устаревшее) |
...
## Миграция на Qwen Code

Проект теперь полностью поддерживает Qwen Code (https://github.com/QwenLM/qwen-code.git) в дополнение к Claude Code.

### Что изменилось:

1. **Добавлена поддержка Qwen Code:** Файл `QWEN.md` — основная ориентация для Qwen Code
2. **Полная конфигурация:** Папка `.qwen/` содержит все необходимые настройки
3. **Совместимость:** Все команды workspace доступны в обеих системах
4. **Автоматическая синхронизация:** Скрипт `refresh-commands.sh` адаптирован для Qwen Code

### Как использовать Qwen Code:

1. Установите Qwen Code с https://github.com/QwenLM/qwen-code.git
2. Откройте этот репозиторий в Qwen Code
3. Используйте команды как обычно (`/start`, `/sync`, `/save`, и т.д.)
4. После первого `/sync` перезапустите Qwen Code для загрузки команд мозга

### Дополнительная документация:

- `MIGRATION-TO-QWEN.md` — подробное руководство по миграции
- `QWEN.md` — полная ориентация для Qwen Code
...
Для Qwen Code используйте `.qwen/QWEN.local.md` для локальных заметок.
